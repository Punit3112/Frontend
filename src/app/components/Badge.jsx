...
export default function Badge({ type, name }) {
    ...
    const tooltipText = typeof config.tooltip === "function"
        ? config.tooltip(name)
        : config.tooltip;

    return (
        <div className="flex items-end bg-black/70 mr-[5px] rounded-[5px] p-[5px] border border-white cursor-pointer shadow-[rgba(120,120,170,0.3)_0_2px_10px_0]">
            {config.icon(config.fill)}
            <div className="font-semibold text-[70%] text-white">{config.label}</div>
            <div className="invisible w-[230px] bg-[var(--black)] text-white font-normal text-center rounded-[6px] p-[8px] absolute z-[1] top-[20px] left-[-115px] text-[12px] shadow-[rgba(120,120,170,0.15)_0_2px_16px_0]">{tooltipText}</div>
        </div>
    );
}